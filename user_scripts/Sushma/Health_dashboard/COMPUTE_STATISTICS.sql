...
SELECT SOURCESYSTEM_CD, count(1) as nb_observation_fact, count(distinct patient_num) as distinct_patient_num, count(distinct CONCEPT_CD) as distint_CONCEPT_CD, min(START_DATE) as min_start_date, max(START_DATE) as max_start_date FROM OBSERVATION_FACT where SOURCESYSTEM_CD not like  'EGP%' and SOURCESYSTEM_CD not like  'GSE%' group by SOURCESYSTEM_CD;
