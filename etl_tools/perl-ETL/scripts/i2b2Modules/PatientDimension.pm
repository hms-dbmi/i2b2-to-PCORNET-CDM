...
sub getNewPatientIdList {
	...
	{
		$lastId = DatabaseConnection::getNewIdentifiersLarge($numberOfIdsToGet, "I2B2DEMODATA.SEQ_PATIENT_NUM", $configurationObject);
	}
	
	my $firstId = $lastId - $numberOfIdsToGet;

	print("DEBUG - PatientDimension.pm : Retrieved ($numberOfIdsToGet), First ID ($firstId), Last ID($lastId) \n");
	
	my @returnPatientIdArray = ();
	
	for (my $i=$firstId; $i<=$lastId; $i++) {
		push(@returnPatientIdArray, $i);
	}
